...
## 10. Orbit_Analysis.ipynb — Status

This notebook contains deep data analysis on propagated orbits for Transporter-9 (`2023-174`, 55 on-orbit satellites). It uses cached propagation data from `~/propagation_cache.npz` and historical TLEs from `~/tle_history_cache.json`.

### Completed sections

**Section 1 — Static TLE Analysis ✅**
1. ✅ Inclination distribution — mean 97.4°, std 0.049° — tight SSO cluster, not useful discriminator
2. ✅ Eccentricity distribution — mean 0.000866 — near-circular, not useful discriminator
3. ✅ RAAN distribution — wide spread but noisy, poor discriminator for tight rideshare
4. ✅ Argument of Perigee — 354° range — meaningless for near-circular orbits
5. ✅ BSTAR distribution — mean 0.000555, std 0.000415, range 20x — **strong discriminator**
6. ✅ Mean motion distribution — normally distributed ~15.2 rev/day — altitude proxy
7. ✅ Epoch age — mean 37.3hrs, one stale TLE flagged (FLOCK 4Q 26 at 52.7hrs)

**Section 2 — Propagated Dynamics ✅ (partial)**
8. ✅ Altitude min/max/mean — range 110-627km fleet-wide, core cluster 400-480km
9. ✅ Orbital period — 87-97 min range, TIGER-5 outlier at 87min (110km altitude)
10. ✅ Altitude decay rate — linear fit, TIGER-5 at -54km/day, most at -0.5 to -1.5km/day
11. ✅ Ground track coverage — all satellites cover ±82.7° latitude — not discriminating for SSO
12. ✅ Object separation distance — mean 8676km at day 17-60 post launch (already dispersed)
13. ❌ **Relative velocity between objects — NOT DONE**

**Section 3 — Cross-Satellite Analysis ✅**
14. ✅ Altitude distribution histogram — natural clusters identified
15. ✅ BSTAR vs decay rate — raw r=-0.5866 after removing maneuverers, R²=0.344
16. ✅ Inclination spread — 0.28° total range — confirms SSO, not discriminating
17. ✅ RAAN drift — noise dominated at this timescale, TLE fitting noise > physical signal

**Section 4 — Ground Station Relevance ❌ NOT STARTED**
18. ❌ Number of passes per satellite over 24hrs
19. ❌ Peak elevation distribution across candidates
20. ❌ Pass duration distribution
21. ❌ Doppler shift range per candidate
22. ❌ Pass window overlap — quantifies identification difficulty

### Remaining work in Orbit_Analysis.ipynb

**Item 13 — Relative velocity between objects:**
For each pair of satellites compute the relative velocity (rate of change of separation distance). This tells you how fast the cluster is dispersing and whether any pairs are converging or diverging. Use RTN frame — along-track relative velocity is most meaningful for pass window timing divergence.

```python
# Approach: finite difference of pairwise separation over time
# relative_velocity[pair][t] = (sep[t+1] - sep[t]) / dt
# Or: project velocity difference onto separation unit vector
# v_rel = np.dot(v_b - v_a, (r_b - r_a) / |r_b - r_a|)
```

**Section 4 — Ground Station Relevance:**
This section is the most critical for the MVP — it directly feeds `Multi_Pass_Predictor.ipynb`. 

Setup needed:
```python
# Ground station — Cal Poly SLO
gs_lat_rad = np.radians(35.30)
gs_lon_rad = np.radians(-120.66)
gs_alt     = 0.097  # km
min_elev   = 10.0   # degrees

# Elevation angle function (already validated in Pass_Predictor.ipynb)
def elevation_angle(lat, lon, alt_km):
...
For each of 55 satellites run elevation over 2000 timesteps. Then:

**Item 18 — Passes per satellite:**
Group consecutive timesteps where elevation > min_elev. Count passes per satellite. Expect ~4-6 passes per satellite per 24hrs for ISS-like LEO.

**Item 19 — Peak elevation distribution:**
For each pass find max elevation. Plot histogram across all satellites. Higher peak elevation = better signal strength = more reliable contact attempt.

**Item 20 — Pass duration distribution:**
End time minus start time per pass. Longer passes = more time to attempt contact = higher identification confidence.

**Item 21 — Doppler shift range:**
For each pass compute Doppler shift curve. The range (max - min shift) tells you how much frequency correction the radio needs. Larger range = faster moving satellite = more distinctive Doppler signature = easier to identify.

```python
# Doppler formula (from Pass_Predictor.ipynb)
f0 = 145.825e6  # Hz — or team's actual frequency
doppler_hz = -f0 * (range_rate / c)  # c in km/s
```

**Item 22 — Pass window overlap:**
For each pair of satellites compute overlap between their pass windows. If satellite A is visible from 10:00-10:06 and satellite B is visible 10:02-10:08, overlap = 4 minutes. High overlap = ambiguous identification from timing alone. This is the ground-truth quantification of identification difficulty.

```python
# Approach: for each timestep, count how many satellites are simultaneously visible
# simultaneously_visible[t] = sum(inside[i][t] for i in range(n_sats))
# Overlap metric = fraction of time where >1 satellite visible simultaneously
```

### Key data available in Orbit_Analysis.ipynb
```python
# From propagation cache
r_teme  # (55, 2000, 3) TEME positions
v_teme  # (55, 2000, 3) TEME velocities
e       # (55, 2000) error codes
lats    # (55, 2000) latitudes
lons    # (55, 2000) longitudes
alts    # (55, 2000) altitudes km
times   # (2000,) astropy Time array

# From TLE cache
tles    # list of 55 dicts with name, line1, line2

# From historical cache
hist_df  # pandas df with norad, name, epoch, line1, line2, days_since_launch
```
...
1. **Complete Orbit_Analysis.ipynb** — Item 13 (relative velocity) and all of Section 4 (ground station relevance items 18-22)
2. **Run Multi_Pass_Predictor.ipynb** — multi-TLE pass prediction for all candidates simultaneously over Cal Poly. This is the core identification interface.
3. **Build contact logging** — record pass attempts (contact/no contact) per candidate per pass window
4. **Integrate discriminators** — combine altitude filter + BSTAR + quality score into unified candidate ranking
5. **Add LLM explanations** — Claude API integration for plain-language explanations of every output
